...
app.use(express.static(path.join(__dirname, "public"))); // or just app.use(express.static("public")) if you are sure that the server will always be run from the root directory of the project. (gives relative path to the "public" directory)
// This part constructs the absolute path to the "public" directory, ensuring that it works correctly regardless of where the server is run from.
...
// Static function automatically serves index.html when we access the root URL ("/") if it exists in the "public" directory. This means that if we have an "index.html" file in the "public" directory, we can simply go to "http://localhost:3000/" and it will serve that file without needing to specify the filename in the URL. This is a convenient feature for serving a default homepage or landing page for our application.
