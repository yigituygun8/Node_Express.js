...
Short for "properties," props are read-only data that are passed from a parent component to a child component. They allow components to communicate with each other and share data. Props are immutable, meaning that a child component cannot modify the props it receives from its parent.

## Hooks

Allows us to use state and other React features within functional components. Some common hooks include:
